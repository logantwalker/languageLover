...
function dataViz(stats){
    ...
    //here is where you can specify the height/width of your graph
    const margin = 60;
    var height;
    var width;

    //this bit of code below handles the mobile responsiveness. 
    if(window.innerWidth <= 600){
        //for mobile screens
        width = (window.innerWidth/1) - 2 * margin; //<-- change the value that "innerWidth" or "innerHeight" is divided by
        height = (window.innerHeight/1.5) - 2 * margin;// to change the width/height of the graph in relation to window size
    }
    else{
        //for screens greater than 600px wide
        width = (window.innerWidth/1.5) - 2 * margin;
        height = (window.innerHeight/1.5) - 2 * margin;
    }
